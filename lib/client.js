window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    const en = {
      ...
      dailyCapBrave: 'Daily cap: Brave',
      dailyCapDuckduckgo: 'Daily cap: DuckDuckGo',
      braveApiKeyEnv: 'Brave credential name',
      compactExtract: 'Compact extracted content',
      compactExtractDesc: 'Strip cookie notices, nav noise and redundant formatting to save tokens.',
      circuitBreakerThreshold: 'Circuit breaker failure threshold',
      circuitBreakerThresholdDesc: 'Consecutive errors before temporarily pausing provider (0 disables).',
      health_circuit_breaker: 'Circuit breaker status',
      ...
      searchProviderOrderDesc: 'Comma-separated order. Allowed: tavily, brave, firecrawl, exa, searxng, duckduckgo.',
    ...
    const zh = {
      ...
      dailyCapBrave: '日限额：Brave',
      dailyCapDuckduckgo: '日限额：DuckDuckGo',
      braveApiKeyEnv: 'Brave 凭证名',
      compactExtract: '紧凑内容提取',
      compactExtractDesc: '去除 cookie 提示、导航噪音和冗余格式以节省 Token。',
      circuitBreakerThreshold: '熔断器失败阈值',
      circuitBreakerThresholdDesc: '暂停提供商前的连续错误数 (0 禁用)。',
      health_circuit_breaker: '熔断器状态',
      ...
      searchProviderOrderDesc: '逗号分隔。可选：tavily, brave, firecrawl, exa, searxng, duckduckgo。',
...
`
    ...
    function HealthBlock({ t }) {
      ...
      return React.createElement('div', { className: 'wg-field' },
        ...
        state.status === 'ready' ? React.createElement('div', null,
          ...
          rows.slice(4),
          state.data.circuitBreaker ? React.createElement('div', { className: 'wg-label', style: { marginTop: 8 } }, t('health_circuit_breaker')) : null,
          state.data.circuitBreaker
            ? Object.entries(state.data.circuitBreaker).map(([k, v]) => React.createElement('div', { key: 'cb-' + k, className: 'wg-desc' },
              k + ': ' + v.state + (v.cooldownRemainingMs > 0 ? (' (cooldown ' + Math.ceil(v.cooldownRemainingMs / 1000) + 's)') : '')))
            : null,
    ...
    function SettingsBody({ ctx, t }) {
      ...
      const defaults = {
        ...
        dailyCapTavily: 0, dailyCapBrave: 0, dailyCapFirecrawl: 0, dailyCapExa: 0, dailyCapSearxng: 0, dailyCapDuckduckgo: 0, dailyCapCrawl4ai: 0, circuitBreakerThreshold: 3, compactExtract: true,
        searchProviderOrder: 'tavily,brave,firecrawl,exa,searxng,duckduckgo',
        ...
        braveApiKeyEnv: 'BRAVE_API_KEY', tavilyApiKeyEnv: 'TAVILY_API_KEY', firecrawlApiKeyEnv: 'FIRECRAWL_API_KEY',
      ...
      return React.createElement('div', { className: 'wg-body' },
        ...
        React.createElement('div', { className: 'wg-field' },
          React.createElement('label', { className: 'wg-check', htmlFor: 'wg-compact' },
            React.createElement('input', { id: 'wg-compact', type: 'checkbox', checked: !!draft.compactExtract, onChange: (e) => setField('compactExtract', e.target.checked) }),
            React.createElement('span', { className: 'wg-label' }, t('compactExtract')),
          ),
          React.createElement('div', { className: 'wg-desc' }, t('compactExtractDesc')),
        ),
        num('wg-circuit-breaker', 'circuitBreakerThreshold', 'circuitBreakerThreshold', 'circuitBreakerThresholdDesc'),
        ...
        num('wg-cap-brave', 'dailyCapBrave', 'dailyCapBrave', 'dailyCapsDesc'),
        ...
        num('wg-cap-ddg', 'dailyCapDuckduckgo', 'dailyCapDuckduckgo', 'dailyCapsDesc'),
        ...
        text('wg-brave', 'braveApiKeyEnv', 'braveApiKeyEnv', 'keysDesc'),

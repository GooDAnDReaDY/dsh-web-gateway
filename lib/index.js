...
//     chain: configurable (default tavily -> brave -> firecrawl -> exa -> searxng -> duckduckgo)
//
//   web_gateway_extract(url)           -> { url, provider, markdown }
//     chain: configurable (default firecrawl -> tavily -> crawl4ai -> jina -> readability)
//
...
import { pruneMarkdown } from './pruning.js'
...
export const name = '@goodandready/dsh-web-gateway'
...
export const Config = z.object({
  ...
  dailyCapBrave: z.number().default(0).description('Daily call cap for brave (0 = unlimited, UTC day).'),
  ...
  dailyCapDuckduckgo: z.number().default(0).description('Daily call cap for duckduckgo (0 = unlimited, UTC day).'),
  ...
  circuitBreakerThreshold: z.number().default(3).description('Consecutive failures before entering cooldown (0 disables).'),
  circuitBreakerCooldownMs: z.number().default(300000).description('Circuit breaker cooldown duration in ms.'),
  compactExtract: z.boolean().default(true).description('Prune noise and boilerplate from extracted markdown.'),
  ...
  duckduckgoEnabled: z.boolean().default(true).description('Enable keyless DuckDuckGo search fallback.'),
  ...
  searchProviderOrder: z.string().default(DEFAULT_SEARCH_ORDER).description(
    'Comma-separated search provider order. Allowed: tavily, brave, firecrawl, exa, searxng, duckduckgo.',
  ),
  extractProviderOrder: z.string().default(DEFAULT_EXTRACT_ORDER).description(
    'Comma-separated extract provider order. Allowed: firecrawl, tavily, crawl4ai, jina, readability.',
  ),
...
export function apply(ctx, config) {
  ...
  let circuitBreaker = makeCircuitBreaker({
    threshold: Number(live?.circuitBreakerThreshold) || 3,
    cooldownMs: Number(live?.circuitBreakerCooldownMs) || 300000,
  })
      ...
      wctx.effect(() => wctx.webServer.register({
        ...
        handler: async (request, response) => {
          try {
            ...
            snap.circuitBreaker = circuitBreaker.snapshot()
  ...
  function withGuards(provider, cfg, run) {
    return async () => {
      if (Number(cfg?.circuitBreakerThreshold) > 0) {
        const cbGate = circuitBreaker.check(provider)
        if (!cbGate.ok) return { ok: false, provider, reason: cbGate.reason }
      }
      ...
      try {
        ...
        if (out?.ok) {
          ...
          dailyCaps.record(provider)
          return out
        }
        if (out?.reason) {
          circuitBreaker.recordFailure(provider, out.reason)
        }
        return out
      } catch (err) {
        circuitBreaker.recordFailure(provider, err?.message || String(err))
        throw err
      }
    }
  }
  ...
  const searchRunners = {
    tavily: (cfg, query, limit, depth, signal, filters) => withGuards('tavily', cfg, () => tavilySearch(ctx, cfg, query, limit, depth, signal, fetchImpl, filters)),
    brave: (cfg, query, limit, _depth, signal, filters) => withGuards('brave', cfg, () => braveSearch(ctx, cfg, query, limit, signal, fetchImpl, filters)),
    firecrawl: (cfg, query, limit, _depth, signal, filters) => withGuards('firecrawl', cfg, () => firecrawlSearch(ctx, cfg, query, limit, signal, fetchImpl, filters)),
    exa: (cfg, query, limit, _depth, signal, filters) => withGuards('exa', cfg, () => exaSearch(ctx, cfg, query, limit, signal, fetchImpl, filters)),
    searxng: (cfg, query, limit, _depth, signal, filters) => withGuards('searxng', cfg, () => searxngSearch(ctx, cfg, query, limit, signal, fetchImpl, filters)),
    duckduckgo: (cfg, query, limit, _depth, signal, filters) => withGuards('duckduckgo', cfg, () => duckduckgoSearch(ctx, cfg, query, limit, signal, fetchImpl, filters)),
  }

  const extractRunners = {
    firecrawl: (cfg, url, signal) => withGuards('firecrawl', cfg, () => firecrawlExtract(ctx, cfg, url, signal, fetchImpl)),
    tavily: (cfg, url, signal) => withGuards('tavily', cfg, () => tavilyExtract(ctx, cfg, url, signal, fetchImpl)),
    crawl4ai: (cfg, url, signal) => withGuards('crawl4ai', cfg, () => crawl4aiExtract(ctx, cfg, url, signal, fetchImpl)),
    jina: (cfg, url, signal) => withGuards('jina', cfg, () => jinaExtract(ctx, cfg, url, signal, fetchImpl)),
    readability: (cfg, url, signal) => withGuards('readability', cfg, () => readabilityExtract(ctx, cfg, url, signal, fetchImpl)),
  }
  ...
  registerSafeTool(ctx, defineTool({
    ...
    description:
      'Search the web through a resilient fallback chain (default: tavily -> brave -> firecrawl -> exa -> searxng -> duckduckgo) '
    ...
    parameters: {
      ...
      category: {
        ...
        description: 'Topic/category filter: general | news | code | academic. Default general.',
      },
    ...
    async execute(args, exec) {
      ...
      const category = args.category && ['general', 'news', 'code', 'academic'].includes(args.category)
        ? args.category
        : 'general'
      const filters = {
        category,
      ...
      const order = parseProviderOrder(cfg.searchProviderOrder, SEARCH_PROVIDER_IDS)
      const cacheKey = `gsearch:${query}:${limit}:${depth}:${category}:${order.join('>')}:${filters.includeDomains}|${filters.excludeDomains}|${filters.freshness}`
  ...
  registerSafeTool(ctx, defineTool({
    ...
    description:
      ...
      + '(default: firecrawl -> tavily -> crawl4ai -> jina -> readability). Returns which providers were skipped. '
    ...
    parameters: {
      ...
      compact: { type: 'boolean', description: 'Strip boilerplate notices and prune tokens (default: true).' },
    },
    output: {
      schema: {
        type: 'object',
    ...
    async execute(args, exec) {
      ...
      const isCompact = args.compact ?? cfg.compactExtract ?? true

      const order = parseProviderOrder(cfg.extractProviderOrder, EXTRACT_PROVIDER_IDS)
      const cacheKey = `gextract:${url}:${order.join('>')}:${isCompact ? 'compact' : 'full'}`
      const cached = cache.get(cacheKey)
      if (cached) return { ...cached, cached: true, tookMs: Date.now() - t0 }
      ...
      const rawMarkdown = outcome.payload.markdown || ''
      const cleanMarkdown = isCompact ? pruneMarkdown(rawMarkdown, { compact: true }) : rawMarkdown
  ...
  registerSafeTool(ctx, defineTool({
    ...
    async execute(args, exec) {
      ...
      for (const r of results) {
        ...
        const md = pruneMarkdown(String(extractOutcome.payload.markdown || ''), { compact: true })
        sources.push({
          title: r.title || '',
          url: href,
